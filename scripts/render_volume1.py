...
def addIsosurface(volume, volumeObject, renderView, zexag):
...
def renderAnimation(camera, ortho, volume, path, zexag, numFrames, output):
    ...
    pathpoints = generatePath(((bbox[0] + bbox[1]) / 2, (bbox[2] + bbox[3]) / 2), distance=2*distance, height=300)
...
"""
    ...
    WriteAnimation(os.path.join(output), Magnification=1, FrameRate=20.0, Compression=True)
...
if __name__ == '__main__':
    ...
    renderAnimation(camera, ortho=ortho, volume=volume, path=path, zexag=3, numFrames=200,
                    output=ortho + '.png')
